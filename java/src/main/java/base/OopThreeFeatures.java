...
public static void main(String[] args) {
	...
	/**
	 * 梳理下类之间的关系
	 * base.China 继承于 base.Country
	 * base.China 有 printS() 方法
	 * base.Country 有 printF() 方法
	 */
	System.out.println("\n----- 多态 -----");
	...
	System.out.println("\n..... (1) 父类对象转成子类对象 .....");
	// true
	System.out.println("co1 instanceof base.China: " + (co1 instanceof China));
	// true
	System.out.println("ch1 instanceof base.China: " + (ch1 instanceof China));
	// 子类重新父类的方法，因此下面两结果均为：base.China.method()
	co1.method();
	ch1.method();
	System.out.println("\n..... (2) 父类对象转成子类对象（强制转换） .....");
	// 如果想要让父类获取到子类的特有方法printS()，要将父类对象co1强制转换为子类的类型（高-->低），所以要进行强制类型转换
	// printF() 为父类的方法，父类对象 co1 可以直接访问结果为：County Method；强制转换后父类对象可以访问到子类独有方法，结果为：base.China Method
	co1.printF();
	((China) co1).printS();
	System.out.println("\n..... (3) 子类对象转换为父类对象（自动转换） .....");
	China ch2 = new China();
...
public China(String name) {
	System.out.println("base.China 的有参构造器" + name);
}
...
public void printS() {
	System.out.println("base.China Method");
}
...
public void method() {
	System.out.println("base.Country.method()");
}
