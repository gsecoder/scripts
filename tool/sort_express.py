...
# -*- encoding: utf-8 -*-
"""
__file__    : sort_express.py
__time__    : 2020/6/9 21:15
...
def sort_express(express_info):
    express_dict = {}

    for express in express_info:
        # 获取每条快递的地址信息列
        express_address = express[1][0: 2]


        # 判断地址关键字省部分是否在空字典 express_dict 中
        # 如果存在则将整条信息加入到存放快递人和地址信息的 expresses_dict 的字典中
        if express_address not in express_dict:
            express_dict[express_address] = []
            # print("express_dict[express_address]: ", type(express_dict[express_address]))
            express_dict[express_address].append(express)
        else:
            express_dict[express_address].append(express)

    print("{")
    for index in express_dict:
        print("\t" + index + ": [")
        for i in express_dict[index]:
            print("\t\t" + str(i))
        print("\t], ")
    print("}")


sort_express(express_list)